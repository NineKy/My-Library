...
인덱스가 각 샤드로 나뉘어지면서 여러 노드에 저장이 되는데, 각각의 인덱스별로 데이터의 저장 및 검색 방법에 대해서 저장 및 고려하게 되어 있습니다 <br>
...
```
<br>
이외에도 아래와 같은 설정들이 존재합니다 <br>
- refresh_interval : 세그먼트가 만들어지는 리프레시 타임(defaut 1), replica 설정 수와 동일하게 동적으로 설정하는 것이 가능(떠 있는 상태에서 설정 가능)
...
<br> 
인덱스별로 가장 많이 보이는 기능들은 위와 같고, 각각의 커스터마이징을 통해서 지정하는 것이 가능하기 때문에 사용하는데 있어서 많은 실습을 통해서 확인이 필요해보입니다 <br> 
<br><br>
...
- 동적 매핑은 처음 색인되는 문서를 바탕으로 매핑정보를 ES가 동적으로 매핑
...
- 문자열 - text, keyword
  - text : 해당 타입 같은 경우에는 해당 타입으로 넣어둔 문자열이 **각각의 단어를 단위로 각각 이 토큰화**되는 방식입니다 > keyword보다 속도적으로 빠름
  - keyword : 해당 타입 같은 경우에는 해당 타입으로 넣어둔 문자열이 문자열 **전체가 그대로 토큰화**되는 방식입니다 > 토큰화 되는 것은 /_analyze 을 통해서 토큰화되는 내용 확인 가능
...
<br>
인덱스의 설정과 매핑 작업에 대해서는 필요에 따라서 선택해서 작업하면 될 것으로 보입니다

## 색인 및 검색 과정
ES에서 색인이라는 단어는 데이터를 적재하는 과정을 의미합니다 <br> 
정확하게는 인덱스를 생성하고 매핑을 검증하고 역인덱스를 생성하고 document을 저장하는 과정입니다 <br>
데이터를 저장하는 과정은 샤드의 갯수를 설정하는 것이 중요한 요소인 점을 기억해두면 좋을 것으로 보입니다 추가로 해당 내용은 처음부터 완벽한 샤드의 설정을 하는 것은 어려우니 유스케이스에 맞춰서 차차 맞춰가는 것이 중요합니다 <br>
색인의 성능을 고려할 때에는 가능한 많은 데이터 노드들이 색인 과정에 참여할 수 있도록 클러스터 구조로써 잘 동작하는지에 대해서 중점적으로 보면 좋습니다 <br>
데이터 색인, 즉 데이터를 적재하게 되면 text 필드별로 역 인덱스를 생성합니다 여기서 역 인덱스는 각 단어별로 어떤 document에서 해당 단어를 쓰는지 리스팅을 해두고 그 상태로 저장하게되는 구조입니다 <br>
그 상태에서 문자열을 토크나이징을 통해서 저장하는데 해당 방식은 캐릭터필터 > 토크나이저 > 토큰필터 순으로 진행됩니다 <br>
캐릭터필터는 전체 문장에서 특정 문자를 대체하더가 제거 하는 역할을 진행합니다 <br>
토크나이저는 설정한 토크나이저를 베이스로(하나만 지정가능) 단어를 분리합니다 <br>
마지막을 토큰필터는 토크나이저를 통해서 분리된 단어를 필터로 지정한 규칙에 따라서 처리합니다 <br>
이후에는 추가적으로 형태소분석을 통해서 또 다시 구분하는 것이 가능한데 예시로써는 nori(한글 형태소분석)이 가능합니다 <br>
<br><br>

검색 과정은 프라이머리 샤드에서만 가능했던 것과 달리, 검색 과정은 프라이머리, 복제 샤드 모두에서 가능합니다 <br>
